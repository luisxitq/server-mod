...
string whois(const void* address, bool include_offset = false) {
    Dl_info info;
    char buf[256]; // ফরম্যাট করা টেক্সট রাখার জন্য বাফার

    if (!dladdr(address, &info)) {
        if (include_offset) {
            // std::format এর বদলে snprintf ব্যবহার
    ...
    // লাইব্রেরির নাম এবং মেমরি অফসেট ফরম্যাট করা
    snprintf(buf, sizeof(buf), "%s(0x%lx)", libname.c_str(), (unsigned long)offset);
    return string(buf);
}

string whois(ptr address, bool include_offset = false) { 
